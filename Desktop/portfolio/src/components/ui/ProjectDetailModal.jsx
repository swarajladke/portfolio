...
const ProjectDetailModal = ({ isOpen, onClose, project }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overscroll-contain animate-fade-in" onClick={handleBackdropClick}>
      {/* Make the modal itself the scroll container so mouse wheel always works when hovered */}
      <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto overscroll-contain touch-pan-y bg-card rounded-2xl border border-border animate-scale-in pointer-events-auto" onWheelCapture={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-border sticky top-0 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/70 z-10">
